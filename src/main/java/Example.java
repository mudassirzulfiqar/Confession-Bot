    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            JDA jda = JDABuilder.createDefault("ODA5NDg1NzM5MDYyNTkxNTEw.YCVyVw.JeOdbC2b0olwbuVxWvVywZhpdXQ") // The token of the account that is logging in.
                    .addEventListeners(new Example())   // An instance of a class that will handle events.
                    .build();
...
//                privateChannel.sendMessage("Mention Pupho's Channel please").queue();
//                return;
//            }
...
//            TextChannel textChannel = event.getMessage().getMentionedChannels().get(0);
                ...
                event.getJDA().getTextChannelById(puphoChannelId).sendMessage(msg).queue();
            }
...
*/
            if (event.getMember().hasPermission(Permission.ADMINISTRATOR)) {
                event.getGuild().createTextChannel("Pupho").setTopic("Confessional channel created. PM your confession Bot").queue(confessChannel -> {
                    puphoChannelId = confessChannel.getId();
                    System.out.println(puphoChannelId = confessChannel.getId());
                    confessChannel.sendMessage("The Code for the channel is " + puphoChannelId).queue();
                });

            }
        } else if (msg.contains("!pupho")) {
//            System.out.println("Confession: " + msg.replace("!pupho", ""));
//            TextChannel channelToConfess = event.getGuild().getTextChannelById(puphoChannelId);
//            if (channelToConfess != null) {
//                channelToConfess.sendMessage(event.getMessage().getContentRaw().replace("!pupho", ""))
//                        .delay(1, TimeUnit.SECONDS)
//                        .flatMap(Message::delete).queue();
//        }
        } else if (msg.contains("!magic")) {
//            event.getAuthor().openPrivateChannel() // RestAction<PrivateChannel>
//                    .flatMap(_channel -> _channel.sendMessage("fuck you")) // RestAction<Message>
...
//                    .flatMap(Message::delete); // RestAction<Void> (executed 30 seconds after sending)*/
        } else if (msg.equals("!channelsId")) {
            String channelIds = message.getGuild()
                    .getChannels()
                    .stream()
                    .map(ISnowflake::getId)
                    .collect(Collectors.joining(","));
            channel.sendMessage(channelIds).queue();
